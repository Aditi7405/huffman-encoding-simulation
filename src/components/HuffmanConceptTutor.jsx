...
export default function HuffmanConceptTutor({ open, onClose }) {
  ...
  {
    title: "Merge Phase",
    text: "In Merge phase, selected nodes combine into a new parent node. This repeats until one root node remains.",
    ref: prevStepBtnRef,
  },
  {
...
const handleSymbolSelected = (index) => {
  ...
  const u = new SpeechSynthesisUtterance(`You selected ${name}. Now click Analyze Frequency.`);
  u.rate = 0.95;
  u.onend = () => {
    ...
    <Dialog
      ...
      <DialogTitle id="instructions-dialog-title"
        ...
        <div style={{ position: 'relative', display: "flex", alignItems: "center", gap: '10px' }}>
          ...
          <Button title="Pause"
            onClick={() => { window.speechSynthesis.cancel(); setIsHuffmanSpeaking(false); }}
...
{isConceptTourRunning && (() => {
  const currentRef = conceptTourSteps[conceptStep]?.ref;
  const pos = getPopupPos(currentRef);
  return (
    <Portal>
      <div style={{
        position: 'fixed',
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', gap: '10px' }}>
          ...
          if (currentStepData?.waitForSymbol) {
          setWaitingForSymbol(true);  // hint dikhao
          return;
          }
          ...
          const next = conceptStep + 1;
          setConceptStep(next);
          if (conceptTourSteps[next]?.waitForAnalyze) {
          setWaitingForAnalyze(true);
      }
      window.speechSynthesis.cancel();
      const u = new SpeechSynthesisUtterance(conceptTourSteps[next].text);
